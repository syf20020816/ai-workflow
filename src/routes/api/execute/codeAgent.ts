  ...
  server: {
    handlers: {
      POST: async (ctx: any) => {
        ...
        // 应用地图（App-Desc）：使用者自行生成并在节点配置路径，平台只读取注入，不生成
        let appMapSection = ''
        if (appMapPath && projectPath) {
          const projectRoot = path.isAbsolute(projectPath)
            ? path.resolve(projectPath)
            : path.resolve(process.cwd(), projectPath)
          appMapSection = await readAppMap(appMapPath, projectRoot, logs)
        }
...
async function handleBatchMode(input: BatchModeInput) {
  ...
  // 应用地图（App-Desc）：使用者自行生成并在节点配置路径，平台只读取注入，不生成
  if (appMapPath) {
    const appMapSection = await readAppMap(appMapPath, projectRoot, logs)
    if (appMapSection) {
...
/**
 * 读取使用者提供的应用地图文件（appMapPath 支持绝对路径或相对项目根目录的路径）。
 * 平台不自动生成：文件不存在/读取失败返回空字符串（仅记日志，不阻塞执行）。
 */
async function readAppMap(
  ...
  if (!isPathSafe(appMapPath)) {
    logs.push(`应用地图路径不安全（含 ..），已跳过: ${appMapPath}`)
    return ''
  }
  const resolved = path.isAbsolute(appMapPath)
    ? path.resolve(appMapPath)
    : path.resolve(root, appMapPath)
  try {
    const content = await fs.readFile(resolved, 'utf-8')
    if (content.trim()) {
      logs.push(`已加载应用地图: ${appMapPath}`)
      return `## 项目应用地图（${appMapPath}）\n${content.trim().slice(0, 20000)}`
    }
    logs.push(`应用地图文件为空: ${appMapPath}（已跳过）`)
  } catch {
    logs.push(`应用地图不存在或读取失败: ${appMapPath}（已跳过，平台不自动生成）`)
  }
  return ''
}

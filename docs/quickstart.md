...
本指南带你从零开始，在 5 分钟内用可视化画布搭建并运行第一个 AI 工作流。

## 1. 环境准备

- **Node.js** ≥ 20（AI SDK 与构建工具要求）
- **包管理器**：npm 或 pnpm 均可
- **可选**：`lark-cli`（使用 Lark 节点时需要，宿主机关机后需重新 `lark-cli auth login`）
- **可选**：Qdrant 向量库（使用知识库节点时需要）
...
# 生成路由（新增 API/页面路由后需要，dev 模式通常自动生成）

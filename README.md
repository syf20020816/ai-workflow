...
1. **P0** — BMad CLI 实际调用（executor 从 mock 改为真调用）
2. **P0** — Lark CLI 实际对接（executor 从 mock 改为真调用）
3. **P1** — Qdrant 向量检索节点 + 语义检索节点
4. **P1** — 工作流 JSON 序列化（保存/加载）
5. **P2** — BMad 全流程映射（将画布编排转换为 BMad Method 的 planning → sprint → development 循环）
6. **P2** — 条件分支/循环/并行执行等高级编排能力
